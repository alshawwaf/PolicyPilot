...
A disable-undo is deliberately **not terminal**: the change record enters a `disabled` state (rule off but still present) from which it can be **finalized** — delete the disabled rule outright — or **re-enabled** (an added rule returns to `active` and is rollable again; re-enabling a rule that a *removal* disabled restores the original access, which is terminal). The state machine (`active → resolved | disabled`, `disabled → resolved | active`) lives once in `change_log.revert_state()` and is shared verbatim by the portal's change panel, the MCP tool (`revert_change` with `disable_instead_of_delete` / `delete_rule` / `reenable`), and the REST endpoint (`POST /dbapi/v1/access/revert`) — the three surfaces can never disagree about what an entry allows. Transitions are **claimed atomically before the SMS is touched** (one actor per entry, restored on SMS failure), so a committed publish is stamped by construction and a failed one is never falsely recorded as resolved.
