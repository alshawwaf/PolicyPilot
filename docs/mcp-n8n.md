...
PolicyPilot exposes its access-automation brain as **31 MCP tools** an LLM agent (via n8n's *MCP Client
...
31 tools across two rails. The **Writes?** column notes which gate (if any) controls live writes — the two
...
| `list_unused_objects(server_id)` | objects nothing references (cleanup candidates), grouped by type | no |
