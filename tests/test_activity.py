...
from app.routers.activity import KIND_LABELS, PAGE_SIZES, _filter_conds
...
def _row(rid=1, kind="api"):
    return type("R", (), {"id": rid, "at": dt.datetime(2026, 6, 16, 23, 24, 45), "kind": kind,
                          "method": "GET", "path": "/dbapi/v1/x", "source_ip": "1.2.3.4",
...
def test_shell_renders_checkbox_filters_page_size_and_modal():
    counts = {"all": 4445, "layer_apply": 19, "api": 4077}
    html = _render("activity.html", counts=counts, kind_labels=KIND_LABELS, selected=["layer_apply"],
                   page_size=10, page_sizes=PAGE_SIZES, q="",
                   selected_status=[], status_classes=["2xx", "3xx", "4xx", "5xx"], flash=None)
    # filters are checkboxes on the left; the selected one is checked
    assert 'name="kinds" value="layer_apply" class="kind-cb" checked' in html
    assert 'name="kinds" value="api" class="kind-cb" ' in html
    ...
    # Status / Type are multiselect dropdown menus folded into the top toolbar (no data-center filter)
    assert 'id="msel-status"' in html and 'id="msel-type"' in html
    assert 'id="msel-dc"' not in html and 'name="dc"' not in html
    assert 'name="status" value="4xx"' in html


def test_filter_conds_are_independent_and_filters():
    # kinds / status / q are independent AND conditions (each OR within itself).
    assert len(_filter_conds([], "", [])) == 0                          # nothing → no filter
    assert len(_filter_conds(["layer_apply"], "", [])) == 1             # kinds only
    assert len(_filter_conds([], "", ["4xx"])) == 1                     # status only
    assert len(_filter_conds(["api"], "", ["4xx"])) == 2               # kinds AND status (independent)
    assert len(_filter_conds(["layer_apply"], "403", ["4xx"])) == 3     # kinds + search + status
    assert len(_filter_conds([], "sdk", [])) == 1                       # search only
...
def _seed_db():
    ...
    a = ActivityLog(kind="api", path="/a")
    b = ActivityLog(kind="api", path="/b")
...
def test_clear_by_kinds_deletes_only_those_categories():
    db, a, b, c = _seed_db()
    db.execute(delete(ActivityLog).where(ActivityLog.kind.in_(["api"])))
    db.commit()
